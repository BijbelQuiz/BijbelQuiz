...
/// Enhanced GameStatsProvider using the centralized sync service
class GameStatsProvider extends ChangeNotifier {
  ...
  Future<void> _loadStats() async {
    try {
      ...
      if (CentralizedSyncService.instance.isInRoom) {
        await CentralizedSyncService.instance.syncData('game_stats', getExportData());
      }
  ...
  Future<void> updateStats({required bool isCorrect}) async {
    try {
      ...
      if (CentralizedSyncService.instance.isInRoom) {
        await CentralizedSyncService.instance.syncData('game_stats', getExportData());
        AppLogger.debug('Game stats synced to room');
      }
  ...
  /// Loads game stats data from import (for sync)
  Future<void> loadImportData(Map<String, dynamic> data) async {
    try {
      ...
      AppLogger.info('Game stats loaded from sync: score changed from $oldScore to $_score');
      notifyListeners();
    } catch (e) {
      AppLogger.error('Failed to load game stats from sync data', e);
    }
  }

  /// Sets up sync listener for real-time updates
  void _setupSyncListener() {
    CentralizedSyncService.instance.addListener('game_stats', loadImportData);
  }
  ...
  Future<List<String>?> getDevicesInRoom() => CentralizedSyncService.instance.getDevicesInRoom();

  /// Gets the current device ID
  Future<String> getCurrentDeviceId() => CentralizedSyncService.instance.currentDeviceId;

  /// Removes a specific device from the current room
  Future<bool> removeDevice(String deviceId) => CentralizedSyncService.instance.removeDevice(deviceId);

  /// Forces immediate sync of current stats
  Future<void> forceSync() async {
    if (CentralizedSyncService.instance.isInRoom) {
      await CentralizedSyncService.instance.syncData('game_stats', getExportData());
      AppLogger.info('Forced sync of game stats');
    }
  }
}
